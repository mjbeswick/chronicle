...
from models.storage import FileStorage, slugify
...
class FileStorageTests(unittest.TestCase):
    ...
    def test_journal_entry_round_trip_and_rename(self) -> None:
        ...
        entry = self.storage.create_journal_entry(
        ...
        updated = self.storage.update_journal_entry(
            entry.id,
            title="Evening Notes",
    ...
    def test_todo_history_round_trip_and_toggle(self) -> None:
        ...
        completed = self.storage.toggle_todo(
            todo.id,
            True,
            occurred_at=datetime(2026, 4, 21, 10, 0, tzinfo=timezone.utc),
        )
        self.assertTrue(completed.completed)
        reopened = self.storage.toggle_todo(
            todo.id,
            False,
    ...
    def test_timeline_mixes_journal_entries_and_todo_activity(self) -> None:
        ...
        self.storage.toggle_todo(
            todo.id,
            True,
            occurred_at=datetime(2026, 4, 21, 10, 30, tzinfo=timezone.utc),
        )
        self.storage.create_journal_entry(
            title="Afternoon Notes",
            content="Checked in after lunch.",
