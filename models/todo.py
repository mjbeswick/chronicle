...
class TodoHistoryEvent:
    ...
    def timeline_label(self, todo: "TodoItem") -> tuple[str, str]:
        """Return a concise title/summary pair for the journal timeline."""
        action = self.action
        if action == "created":
...
class TodoItem:
    ...
    def __post_init__(self) -> None:
        ...
        self.slug = self.slug.strip() or _slugify(self.title)
        ...
        self.completed_at = _parse_datetime(self.completed_at) if self.completed_at else None
        self.history = [event if isinstance(event, TodoHistoryEvent) else TodoHistoryEvent.from_dict(event) for event in self.history]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "due_date": _dump_date(self.due_date),
            "completed": self.completed,
            "created_at": _dump_datetime(self.created_at),
            "updated_at": _dump_datetime(self.updated_at),
            "completed_at": _dump_datetime(self.completed_at) if self.completed_at else None,
    ...
    def from_dict(cls, payload: Mapping[str, Any]) -> "TodoItem":
        return cls(
            ...
            slug=str(payload.get("slug") or _slugify(str(payload.get("title") or "todo"))),
            ...
            completed_at=_parse_datetime(payload["completed_at"]) if payload.get("completed_at") else None,
        ...
        *,
        ...
        if title is not None:
            ...
            if normalized_title != self.title:
                ...
                self.slug = _slugify(normalized_title)
        ...
        if due_date is not _UNSET:
            normalized_due_date = _parse_date(due_date if due_date is not _UNSET else None)
            if normalized_due_date != self.due_date:
                previous = _dump_date(self.due_date)
                self.due_date = normalized_due_date
                self.record_event(
                    "due_date_changed",
                    field="due_date",
                    before=previous,
                    after=_dump_date(normalized_due_date),
                    occurred_at=when,
                )
                changed = True
